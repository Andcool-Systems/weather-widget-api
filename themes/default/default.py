...
class DefaultTheme:
    def __init__(self, weather_object: wthr.Weather, language: str, timezone):
        self.timezone = timezone
        self.supported_language = ["ru", "en"]
        self.weather = weather_object
        self.language = language

    def addZero(self, num: int) -> str:
        if num < 10:
            return '0' + str(num)
        return str(num)
    ...
    def image(self) -> Image:
        ...
        with open("themes/default/resources/lang.json", "r", encoding="utf-8") as file:
            lang = json_encode_file(file)

        # Load fonts and image
        fnt_big = ImageFont.truetype("themes/default/resources/manrope-bold.ttf", 25)
        fnt_med = ImageFont.truetype("themes/default/resources/manrope-bold.ttf", 15)
        fnt_small = ImageFont.truetype("themes/default/resources/manrope-bold.ttf", 13)
        fnt_med_small = ImageFont.truetype("themes/default/resources/manrope-bold.ttf", 11)
        fnt_very_small = ImageFont.truetype("themes/default/resources/manrope-bold.ttf", 7)
        windArrow = Image.open("themes/default/resources/wind.png")
        ...
        # Formatting time
        timeFormatted = f"{lang['at_the_moment'][self.language]} {nowTime.hour}:{self.addZero(nowTime.minute)} UTC{newTimezonePreview[-2:]}"

        # Get icon
        weatherIcon = Image.open(f"themes/default/icons/{self.weather.weather_icon_name}.png").resize((90, 90))

        # Specify dimensions
        width, height = 345, 145

        # Load image
        mainImage = Image.open("themes/default/resources/background.png")
        mainImage.paste(weatherIcon, (0, height // 2 - 90 // 2), weatherIcon)  # Paste weather icon to canvas
        line = Image.open("themes/default/resources/line.png")
        ...
        sizeCounter = 15
        detailedStatusFont = ImageFont.truetype("themes/default/resources/manrope-bold.ttf", sizeCounter)
